...
// Same "status drives color" convention as purchaseOrderPdf.ts's STATUS_COLOR - kept as a
// separate map (not shared) since the two document types have entirely different status enums.
const STATUS_COLOR: Record<SalesOrderStatus, { bg: readonly [number, number, number]; text: readonly [number, number, number] }> = {
    Draft: { bg: [226, 232, 240], text: [71, 85, 105] },
    Confirmed: { bg: [219, 234, 254], text: [37, 99, 235] },
    Processing: { bg: [254, 243, 199], text: [217, 119, 6] },
    'Partially Shipped': { bg: [237, 233, 254], text: [124, 58, 237] },
    Dispatched: { bg: [220, 252, 231], text: [22, 163, 74] },
...
// Mirrors purchaseOrderPdf.ts's exportPurchaseOrderPdf layout exactly (same header/badge/
...
export function exportSalesOrderPdf(so: SalesOrder, logoDataUrl?: string | null, companySettings?: { companyName: string; address: string }): void {
    ...
    // ---- Header: logo only (left), big centered title, status badge (right) ----
    drawPdfLogo(doc, logoDataUrl, MARGIN, MARGIN - 6, 20);

    doc.setFont('helvetica', 'bold');
    doc.setFontSize(22);
    doc.setTextColor(...COLOR.navy);
    doc.text('SALES ORDER', CENTER_X, y, { align: 'center' });

    const statusColors = STATUS_COLOR[so.status];
    const badgeLabel = so.status.toUpperCase();
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(9);
    ...
    doc.text(formatDate(so.orderDate), badgeX + badgeWidth / 2, MARGIN + 6, { align: 'center' });

    y += 8;
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(11);
    doc.setTextColor(...COLOR.blue);
    doc.text(`#${so.soNo}`, CENTER_X, y, { align: 'center' });
    ...
    // ---- Left column: company address + customer details ----
    ...
    sectionLabel('Customer Details', MARGIN, y);
    y += 6;
    doc.setFont('helvetica', 'bold');
    ...
    // ---- Right column: SO Date / Expected Delivery / Payment Terms, each divided by a rule ----
    let ry = colLeftTop;
    ...
    rightRow('SO Date', formatDate(so.orderDate));
    rightRow('Expected Delivery', formatDate(so.deliveryDate));
    ...
    if (so.deliveryAddress) {
        ...
        doc.setFontSize(9.5);
        doc.setTextColor(...COLOR.body);
        const addrLines = doc.splitTextToSize(so.deliveryAddress, CONTENT_WIDTH);
    ...
    // ---- Notes: flows normally right after the table (Grand Total/footer are pinned near
    ...
    if (so.remarks) {
        ...
        doc.setFontSize(9.5);
        doc.setTextColor(...COLOR.body);
        const remarksLines = doc.splitTextToSize(so.remarks, CONTENT_WIDTH / 2 - 10);
    ...
    doc.setFontSize(10);
    doc.setTextColor(...COLOR.navy);
    doc.text(so.createdBy || '—', MARGIN, footerY + 5.5);
