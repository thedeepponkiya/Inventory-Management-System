...
-- GST No on the customer master record.
ALTER TABLE ims_customer ADD COLUMN IF NOT EXISTS "gstNo" VARCHAR(20);
...
-- Manually typed in by whoever dispatches the order (e.g. the physical delivery challan/bill
-- number) - required at the application layer (dispatchSalesOrder rejects a dispatch without
-- one), not unique. Left nullable here rather than NOT NULL since ADD COLUMN on an
-- already-live table can't retroactively backfill a value for existing dispatch rows.
ALTER TABLE ims_sales_order_dispatches ADD COLUMN IF NOT EXISTS "billNo" VARCHAR(100);
...
-- True only for an invoice created by createInvoiceFromMaterialInward/createInvoiceFromSalesOrder
-- (invoice.controller.js), never toggled afterwards. Lets salesOrder.controller.js's
-- revertDispatch find and clean up ONLY the invoices it auto-created for this SO's dispatches,
-- instead of matching any invoice that merely happens to share the same referenceNo+invoiceType -
-- previously a manually-created Sales invoice a user typed the SO's soNo into as its reference
-- (for their own unrelated bookkeeping reasons) could get silently deleted by a revert it had
-- nothing to do with. Defaults false for every pre-existing row (safe direction: an invoice
-- created before this column existed is never auto-cleaned-up, even if it actually was
-- auto-generated - it just has to be removed manually going forward, same as a genuinely
-- manual invoice always did).
ALTER TABLE ims_invoices ADD COLUMN IF NOT EXISTS "autoGenerated" BOOLEAN NOT NULL DEFAULT false;
